...
#Calibration + physical params
calib=plt.imread('LSH0500427_850nm_calibration.bmp') #calibration phase map of the SLM
calib=calib*0.031*np.ones(calib.shape)-0.29*np.ones(calib.shape) #LUT coeffs
...
class Retriever:
    def __init__(self, **kwargs):
        """
        :param kwargs: alg / i_tgt / p_output / iterations / mixing / offset / propagator / phi_0 / pad
        **alg : str Algorithm choice either 'GS', 'MRAF' or 'OMRAF'. Default is GS
        **i_tgt : str  target intensity (png image) path / or folder for averaging
        **p_output : str name and path of the file in which to save the phase profile (dummy variable, this test code does
          not save anything)
        **iterations : int number of iterations of the GS loop (default is 5)
        **mixing : float mixing parameter between two iterations. Next iteration is m*iteration(n)+(1-m)*iteration(n-1)
        **offset : float introduces an offset to shift the lowest intensity to a non-zero value. Improves convergence.
        **propagator : str propagator for the simulation of light propagation either angular formalism ("Helmholtz") or
          simple Fourier transform ("Fourier"). Default is "Fourier".
        **phi_0 : initial phase guess (PNG image file). Can also be lens(x) where x is the focal length. In this case
          the iniial phase will be the parabolic profile of a lens of focal length x. This usually improves convergence
          a bit and avoids vortices.
        **pad : boolean. Does zero padding. Do not remember exactly how I implement it but must be useful somehow for
          sampling reasons.
        """
        for args in kwargs:
                    print(args,'',kwargs[args])
        self.alg = kwargs['alg']
        self.i_tgt = kwargs['i_tgt']
        self.p_output = kwargs['p_output']
        #TODO etc ...

    def __setattr__(self, key, value):
        #TODO sets the attributes of the class
        pass
    #Defining the phase term associated with the Helmholtz propagator
    def theta_helm(self, z):
    ...
    def compute_cgh(self, alg,i_tgt,p_output,**kwargs):
        """

        **alg : Algorithm choice either GS, MRAF or OMRAF. Default is GS

        **i_tgt : target intensity (png image)

        **p_output : name of the file in which to save the phase profile (dummy variable, this test code does not save
          anything)
        ...
        **pad : boolean. Does zero padding. Do not remember exactly how I implement it but must be useful somehow for sampling reasons.
        ...
        #Core of the calculation
            ...
            if alg=='MRAF':
                ...
                #TODO
                #implement classical GS
                #include averaging over the set of randomly modulated target intensities
        #Converting all the angles to image format via the look up table
            ...
            print('This function only supports GS, MRAF and OMRAF algorithms')
    
def randomizer(i_in : np.ndarray, K : int, size : tuple) -> np.ndarray:
    """
    :param i_in: input intensity to modulate by a random phase mask
    :param K : number of modulated samples to generate
    :param size: size of the "seed" matrix. This seed matrix will the be interpolated up to the input intensity size
    :return: modulated intensities
    """
    #TODO
    #generate random matrices
    #Propagate i_in*random_phasemap to CCD plane
    #Return propagated images
