...
cdef class WISH_Sensor:
    cdef public float d_SLM
    cdef public float d_CAM
    cdef public float wavelength
    cdef public float z
    cdef public int N_gs
    cdef public int N_mod
    cdef public int N_os
    cdef public float mod_intensity
    ...
    def define_mask(self, np.ndarray I, bool plot):
        """
        ...
        """
        assert I.dtype == DTYPE and plot.dtype==bool
        cdef float threshold = self.threshold
        cdef int h = I.shape[0]
        cdef int w = I.shape[1]
        cdef np.ndarray mask_sr = np.zeros((h, w), dtype=DTYPE)
        # detect outermost non zero target intensity point
        cdef np.ndarray non_zero = np.array(np.where(I > self.threshold))
        cdef np.ndarray non_zero_offset = np.zeros(non_zero.shape, dtype=DTYPE)
        ...
        cdef np.ndarray R_non_zero = np.sqrt(non_zero_offset[0] ** 2 + non_zero_offset[1] ** 2)
        cdef tuple R_max = np.where(R_non_zero == np.max(abs(R_non_zero)))[0][
            0]
