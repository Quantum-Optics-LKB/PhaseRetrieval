...
def Checkerboard(sq:int = 20):
    ...
    """
    h, w = 1080, 1920
    x = np.zeros((h, w), dtype=bool)
    # x[1::2, ::2] = True
    # x[::2, 1::2] = True
    P = [[True,False][(i//sq)%2] for i in range(w)]
    x[[i for i in range(h) if (i//sq)%2 == 0]] = P
    P = [[False,True][(i//sq)%2] for i in range(w)]
    x[[i for i in range(h) if (i//sq)%2 == 1]] = P
    return x
