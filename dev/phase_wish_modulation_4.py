...
class WavefrontSensor:
    ...
    def define_mask(self, I: np.ndarray, plot: bool):
        ...
        R_max = np.where(R_non_zero == np.max(R_non_zero))[0][
            0]
    ...
    def phase_retrieval_wish(self, I0: np.ndarray, I_target: list, Phi_m: list, unwrap: bool = False, plot: bool = True, **kwargs):
        ...
        def _GS_iterate_mod_1(self, phi, Phi_m, I_target, k_s, return_dic):
            ...
            A_s = self.frt(A_f, d_3, -z)  # Propagate back to the near field
            pm_s = np.angle(A_s)
            return_dic[str(k_s)] = pm_s - Phi_m[k_s]
    ...
    def generate_images(self, I0, phi0):
        ...
        for k in range(self.N_mod):
            ...
            phi_m = self.modulate(phi0.shape)
            Phi_m.append(phi_m)
            # Phi_m.append(-phi_m)
        T0 = time.time()
        ...
        for phi_m in Phi_m:
            ...
            A = self.frt(A0, self.d_CAM, self.z)
            I = np.abs(A) ** 2
            """
            fig = plt.figure()
            ...
            ax2.set_title("Propagated intensity with mod")
            ax1.imshow(I0)
            ax2.imshow(I)
            plt.show()
            """
...
I0 = np.asarray(Image.open("intensities/harambe_512.bmp"))[:, :, 0]  # extract only the first channel
phi0 = np.asarray(Image.open("phases/calib_512.bmp"))
I0 = Sensor.gaussian_profile(I0.astype(float)/256, 0.5)
